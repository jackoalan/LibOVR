...
static HMODULE createShim( LPCSTR lpLibFileName, OVRTargetAPI targetAPI )
{
	...
	if( !result )
	{
        MessageBoxW(nullptr, L"Unable to find the Rift Display Driver.", L"Configuration Error", MB_OK | MB_ICONERROR);
...
bool checkUMDriverOverrides(void* context)
{
    ...
    if (loadFunc == NULL)
    {
        MessageBoxW(nullptr, L"Unable to load the Oculus Display Driver. Please reinstall the Oculus Runtime.", L"Configuration Error", MB_OK | MB_ICONERROR);
        return false;
    }
    ...
    if (getRTFilterVersionFunc == NULL)
    {
        ...
        MessageBoxW(nullptr, message, L"Configuration Error", MB_OK | MB_ICONERROR);
        return false;
    }
